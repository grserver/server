...
*      Author: Valkyra
*   Village Unlock, FRS Join, FRS Quit Added by Xela on 09/11/2015
*/
...
void ForceShrineMenuComponent::fillObjectMenuResponse(SceneObject* sceneObject, ObjectMenuResponse* menuResponse, CreatureObject* player){
    ...
    if (ghost->getAdminLevel() == 16) {
        menuResponse->addRadialMenuItem(220, 3, "God Only");
        menuResponse->addRadialMenuItemToRadialID(220, 221, 3, "Random Trainer"); // SWGemu Trainer Method
        menuResponse->addRadialMenuItemToRadialID(220, 222, 3, "Static Trainer"); // Static Jedi Trainer
        ...
        menuResponse->addRadialMenuItemToRadialID(220, 226, 3, "Set Faction Status 1"); // Show Faction Status
        menuResponse->addRadialMenuItemToRadialID(220, 227, 3, "Set Faction Status 2"); // Show Faction Status
    }
}
...
int ForceShrineMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* creature, byte selectedID) {
    ...
    if (selectedID == 213) {
        if (!creature->hasSkill("force_title_jedi_rank_02") && (ghost->getJediState() >= 1)) {
            if (ghost->getJediState() > 2) {
                ...
                box->setPromptText("Before Regaining Padawan you must abandon your FRS");
                ghost->addSuiBox(box);
                ...
                ghost->getZoneServer()->getChatManager()->broadcastGalaxy(NULL, "\\#ffb90fIMPERIAL COMMUNICATION FROM THE REGIONAL GOVERNOR: Lord Vader has detected a vergence in the Force.");
                ManagedReference<SuiMessageBox*> box = new SuiMessageBox(creature, SuiWindowType::NONE);
                ...
                Vector3 trainerPositionFinal(-169.45, -4712.58, 0);
    ...
    if (selectedID == 221) {
        ...
        ghost->setTrainerZoneName(zoneName); // For the Waypoint.
        ManagedReference<SuiMessageBox*> box = new SuiMessageBox(creature, SuiWindowType::NONE);
        box->setPromptTitle("Trainer Info");
        box->setPromptText("Zone Name " + zoneName);
        ghost->addSuiBox(box);
        creature->sendMessage(box->generateMessage());
    }
    if (selectedID == 222) {
        ...
        ManagedReference<Zone*> zone = zoneServer->getZone(0);
        /*ManagedReference<SuiMessageBox*> box1 = new SuiMessageBox(creature, SuiWindowType::NONE);
        box1->setPromptTitle("Trainer ZoneID");
        box1->setPromptText(randomZone);
        ghost->addSuiBox(box1);
        creature->sendMessage(box1->generateMessage());
        Vector3 randomTrainer = zone->getCreatureManager()->getRandomJediTrainer();*/
        ...
        box2->setPromptTitle("Trainer Info2");
        box2->setPromptText("Trainer Cords: " + trainerPositionFinal);
        ghost->addSuiBox(box2);
        creature->sendMessage(box2->generateMessage());*/
        ghost->setTrainerCoordinates(trainerPositionFinal);
        ghost->setTrainerZoneName(zoneName); // For the Waypoint.
        ManagedReference<SuiMessageBox*> box3 = new SuiMessageBox(creature, SuiWindowType::NONE);
        box3->setPromptTitle("Trainer Info");
        box3->setPromptText("Zone Name: " + zoneName);
        ghost->addSuiBox(box3);
        creature->sendMessage(box3->generateMessage());
    }
    if (selectedID == 223) {
    ...
    if (selectedID == 225) {
        ...
        box->setPromptText("Your Faction Status is set to 1");
        ghost->addSuiBox(box);
        creature->sendMessage(box->generateMessage());
    }
    if (selectedID == 227) {
